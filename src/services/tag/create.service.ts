...
export const createTagService = async ({ name }: CreateTagDTO): Promise<TagResponse> => {
    const tag = await getTagByName(name)

    if (tag) return Promise.reject({ message: 'Sorry, this tag is already taken', status: StatusCode.CONFLICT });
